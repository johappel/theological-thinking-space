const STORAGE_KEY="tts-prototype-06";
const emptyState=()=>({
 ...
 activePerspective:null,replyTarget:null,lastSignal:"",lastMove:"",threshold:{}
});
...
function render(){
 ...
 $("stream").innerHTML=state.events.map(e=>{
  const canReply=e.author!=="human";
  return `<article class="entry ${e.author==="human"?"human":e.author==="companion"?"companion":"perspective"}">
 ...
 if(state.replyTarget){
  const target=state.events.find(e=>e.id===state.replyTarget.id);
  $("replyContext").classList.remove("hidden");
  $("replyContext").innerHTML=`Antwort an <strong>${esc(labels[state.replyTarget.author]||state.replyTarget.author)}</strong>: ${esc((target?.text||"").slice(0,140))}`;
  $("messageInput").placeholder=`Antwort an ${labels[state.replyTarget.author]||state.replyTarget.author} …`;
 }else{
  $("replyContext").classList.add("hidden");
  $("messageInput").placeholder="Was bewegt dich jetzt?";
 }
 ...
 const readiness={not_yet:"noch im Prozess",possible:"Schwelle wird denkbar",ready:"Schwelle erscheint tragfähig"};
 $("readiness").textContent=readiness[state.readiness]||readiness.not_yet;
...
$("startBtn").onclick=async()=>{
 if(busy)return;
 const text=$("startInput").value.trim();if(!text)return;
 state.startingPoint=text;
 state.events.push({id:newId(),author:"human",text,reply_to:null,addressee:"companion"});
 render();
...
$("sendBtn").onclick=async()=>{
 ...
 $("messageInput").value="";
 let addressee="companion",replyTo=null;
 if(state.replyTarget){
  addressee=state.replyTarget.author;
  replyTo=state.replyTarget.id;
 }else if(state.activePerspective){
  addressee=state.activePerspective;
 }
 state.events.push({id:newId(),author:"human",text,reply_to:replyTo,addressee});
 state.suggestion=null;state.replyTarget=null;render();
 await callThink(addressee,text,replyTo,addressee);
};

$("messageInput").addEventListener("keydown",e=>{if((e.ctrlKey||e.metaKey)&&e.key==="Enter")$("sendBtn").click()});

$("stream").onclick=e=>{
 const b=e.target.closest("button[data-reply-id]");if(!b)return;
 state.replyTarget={id:b.dataset.replyId,author:b.dataset.replyAuthor};
 if(b.dataset.replyAuthor!=="companion")state.activePerspective=b.dataset.replyAuthor;
 else state.activePerspective=null;
 render();$("messageInput").focus();
};

$("perspectiveToggle").onclick=()=>$("perspectivePicker").classList.toggle("hidden");
$("perspectivePicker").onclick=async e=>{
 const b=e.target.closest("button[data-role]");if(!b||busy)return;
 $("perspectivePicker").classList.add("hidden");
 state.activePerspective=b.dataset.role;state.replyTarget=null;render();
 await callThink(b.dataset.role,"");
};
$("suggestionBox").onclick=async e=>{
 const b=e.target.closest("button[data-suggested]");if(!b||busy)return;
 const role=b.dataset.suggested;
 state.suggestion=null;state.activePerspective=role;state.replyTarget=null;render();
 await callThink(role,"");
};

$("candidateList").onclick=e=>{
 const b=e.target.closest("button[data-i]");if(!b)return;
 const i=Number(b.dataset.i),c=state.pendingCandidates[i];if(!c)return;
 if(b.dataset.a==="accept"){
  const bucket=typeToBucket[c.type];
  if(bucket&&!state.thinkingState[bucket].includes(c.text))state.thinkingState[bucket].push(c.text);
 }
 state.pendingCandidates.splice(i,1);render();
};

$("thresholdBtn").onclick=()=>{
 state.mode="threshold";
 const m=[...state.thinkingState.theological_tensions,...state.thinkingState.emerging_testimonies,...state.thinkingState.questions].slice(0,7);
 $("thresholdSummary").innerHTML=m.length?`<strong>Spuren:</strong><ul>${m.map(x=>`<li>${esc(x)}</li>`).join("")}</ul>`:"Der Denkstand ist noch offen.";
 render();
};
$("backBtn").onclick=()=>{state.mode="thinking";render()};
$("snapshotBtn").onclick=()=>{
 state.threshold={changed:$("changedInput").value.trim(),testimony:$("testimonyInput").value.trim(),not_to_prescribe:$("openInput").value.trim(),open_conditions:$("conditionsInput").value.trim()};
 $("snapshot").textContent=JSON.stringify({starting_point:state.startingPoint,thinking_state:state.thinkingState,current_segment:state.currentSegment,active_perspective:state.activePerspective,threshold:state.threshold},null,2);
 $("snapshot").classList.remove("hidden");save();
};
